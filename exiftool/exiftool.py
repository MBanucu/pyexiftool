...
.. note::
    :py:class:`exiftool.helper.ExifTool` class of this submodule is available in the
    ``exiftool`` namespace as :py:class:`exiftool.ExifTool`
...
from .exceptions import (
    ExifToolVersionError, ExifToolRunning, ExifToolNotRunning,
    ExifToolOutputEmptyError, ExifToolJSONInvalidError,
)
...
def _read_fd_endswith(fd, b_endswith: bytes, block_size: int) -> bytes:
    ...
    # this value can be bigger to capture more bytes at the "tail" of the read, but if
    # it's too small, the whitespace might miss the detection
    ...
    while not _get_buffer_end(output_list, endswith_count).strip().endswith(b_endswith):
        if constants.PLATFORM_WINDOWS:
            ...
            # this does NOT work on windows... and it may not work on other systems...
            # in that case, put more things to use the original code above
...
class ExifTool(object):
    ...
    .. warning::
        Note that options and parameters are not checked.  There is no error handling
        or validation of options passed to *exiftool*.
    ...
    def __init__(
        ...
        :param executable: Specify file name of the *exiftool* executable if it is in
            your ``PATH``.  Otherwise, specify the full path to the ``exiftool``
            executable.

            Passed directly into :py:attr:`executable` property.

            .. note::
                The default value :py:attr:`exiftool.constants.DEFAULT_EXECUTABLE` will
        ...
        :param common_args:
            ...
            * ``-G`` (groupName level 1 enabled) separates the output with
              *groupName:tag* to disambiguate same-named tags under different groups.
            * ``-n`` (print conversion disabled) improves the speed and consistency of output, and is more machine-parsable
            ...
            .. note::
                Depending on your use case, there may be other useful grouping levels
                and options.  Search `Phil Harvey's exiftool documentation`_ for
                **groupNames** and **groupHeadings** to get more info.
        ...
        self._last_status: Optional[int] = None  # previous exit status from exiftool
        # (look up EXIT STATUS in exiftool documentation for more information)

        self._block_size: int = constants.DEFAULT_BLOCK_SIZE  # set to default block size

        # these are set via properties
        self._executable: Union[str, Path] = constants.DEFAULT_EXECUTABLE
    ...
    def encoding(self) -> Optional[str]:
        ...
        :setter: Set a new encoding.
            ...
            * If *new_encoding* is None, will detect it from
              ``locale.getpreferredencoding(do_setlocale=False)`` (do_setlocale is set
              to False as not to affect the caller).
    ...
    def common_args(self) -> Optional[List[str]]:
        ...
        Read `Phil Harvey's ExifTool documentation`_ to get further information on
        what options are available / how to use them.
        ...
        :setter: If ``None`` is passed in, sets common_args to ``[]``.  Otherwise, sets the given list without any validation.
            ...
            .. warning::
                No validation is done on the arguments list.  It is passed verbatim to
                *exiftool*.  Invalid options or combinations may result in undefined
                behavior.
    ...
    def config_file(self) -> Optional[Union[str, Path]]:
        ...
        :setter: File existence is checked when setting parameter
            ...
            * Set to ``""`` has special meaning and disables loading of the default
              config file.  See `ExifTool documentation for -config`_ for more info.
    ...
    def config_file(self, new_config_file: Optional[Union[str, Path]]) -> None:
        ...
        if new_config_file is None:
            ...
            # As per exiftool documentation:  Loading of the default config file may be
    ...
    def running(self) -> bool:
        ...
        if self._running:
            ...
            if self._process.poll() is not None:
                ...
                if self._logger:
                    self._logger.warning("Property 'running': ExifTool process was previously running but died")
    ...
    def last_stdout(self) -> Optional[Union[str, bytes]]:
        ...
        .. note::
            ...
            If the most recent call to execute() ``raw_bytes=True``, then this will
            return ``bytes``.  Otherwise this will be ``str``.
    ...
    .. note::
        This can be set and unset (set to ``None``) at any time, regardless of whether
        the subprocess is running (:py:attr:`running` == True) or not.

    :setter: Specify an object to log to.  The class is not checked, but validation
        is done to ensure the object has callable methods ``info``, ``warning``,
    ...
    def set_json_loads(self, json_loads, **kwargs) -> None:
        """
        **Advanced**: Override default built-in ``json.loads()`` method.  The method is
        only used once in :py:meth:`execute_json`
        ...
        .. note::
            This method is designed to be called the same way you would expect to call
            the provided ``json_loads`` method.
        ...
        .. note::
            This can be set at any time, regardless of whether the subprocess is
            running (:py:attr:`running` == True) or not.

        .. warning::
            This setter does not check whether the method provided actually parses json.
            Undefined behavior or crashes may occur if used incorrectly
    ...
    def run(self) -> None:  # noqa: C901
        ...
        already running (:py:attr:`running` == True).  The process is started with
        :py:attr:`common_args` as common arguments, which are automatically included
        ...
        .. note::
            If you have another executable named *exiftool* which isn't Phil Harvey's
            ExifTool, then you're shooting yourself in the foot as there's no error
            checking for that
        ...
        :raises ExifToolVersionError: :py:attr:`exiftool.constants.EXIFTOOL_MINIMUM_VERSION`
        ...
        # If working with a config file, it must be the first argument after the
        # executable per: https://exiftool.org/config.html
        ...
        try:
            ...
            raise ExifToolVersionError(
                f"Error retrieving Exiftool info.  Is your Exiftool version "
                f"('exiftool -ver') >= required version "
                f"('{constants.EXIFTOOL_MINIMUM_VERSION}')?"
            )

        if self._logger:
            self._logger.info(
                f"Method 'run': Exiftool version '{self._ver}' "
                f"(pid {self._process.pid}) launched with args '{proc_args}'"
            )
        ...
        if not self._exiftool_version_check():
            ...
            if self._logger:
                self._logger.error(
                    f"Method 'run': Exiftool version '{self._ver}' did not meet "
                    f"the required minimum version "
                    f"'{constants.EXIFTOOL_MINIMUM_VERSION}'"
                )
    ...
    def terminate(self, timeout: int = 30, _del: bool = False) -> None:
        ...
        if _del and constants.PLATFORM_WINDOWS:
            ...
            try:
                ...
                # print("after comm")
                # TODO a bug filed with Python, or user error... this doesn't seem to
                # work at all ... .communicate() still hangs
                # https://bugs.python.org/issue43784 ,
                # https://github.com/python/cpython/issues/87950... Windows-specific
                # issue affecting Python 3.8-3.10 (as of this time)
            except RuntimeError:
    ...
    def execute(self, *params: Union[str, bytes], raw_bytes: bool = False) -> Union[str, bytes]:
        ...
        :raises UnicodeDecodeError: If the :py:attr:`encoding` is not specified
            properly, it may be possible for ``.decode()`` method to raise this error
        ...
        # there's a special usage of execute/ready specified in the manual which make
        # almost ensure we are receiving the right signal back
        ...
        # these are special sequences to help with synchronization.  It will print
        # specific text to STDERR before and after processing
        ...
        # this can be configured to be one or more chacters... the code below will
        # accomodate for longer sequences: len() >= 1
        SEQ_ERR_STATUS_DELIM = "="
        # a special sequence, ${status} returns EXIT STATUS as per exiftool
        # documentation - only supported on exiftool v12.10+
        seq_err_status = "${status}"

        # ---------- build the params list and encode the params to bytes, if necessary ----------
        cmd_params: List[bytes] = []

        # this is necessary as the encoding parameter of Popen() is not specified.  We
        # manually encode as per the .encoding() parameter
        for p in params:
        ...
        # f-strings are faster than concatentation of multiple strings --
        # https://stackoverflow.com/questions/59180574/string-concatenation-with-vs-f-string
        cmd_params.extend((
            b"-echo4",
            f"{SEQ_ERR_STATUS_DELIM}{seq_err_status}"
            f"{SEQ_ERR_STATUS_DELIM}{seq_err_post}".encode(self._encoding),
        ...
        if self._logger:
            ...
            self._logger.info("Method 'execute': Command sent = {}".format(cmd_params[:-1]))

        # ---------- read output from exiftool process until special sequences reached ----------

        # NOTE:
        #
        # while subprocess recommends: "Use communicate() rather than .stdin.write,
        # .stdout.read or .stderr.read to avoid deadlocks due to any of the other OS
        ...
        if cmd_stderr[-delim_len:] != err_status_delim:
            # exiftool is expected to dump out the status code within the delims...
            # if it doesn't, the class doesn't match expected exiftool output for
            # current version
            raise ExifToolVersionError(
                f"Exiftool expected to return status on stderr, but got "
                f"unexpected character: {cmd_stderr[-delim_len:]} != "
                f"{err_status_delim}"
            )

        # look for the previous delim (we could use regex here to do all this in one
        # step, but it's probably overkill, and could slow down the code significantly)
        # the other simplification that can be done is that, as of this writing:
        # Exiftool is expected to only return 0, 1, or 2 as per documentation
        # you could just lop the last 3 characters off... but if the return status
        # changes in the future, then this code would break
    ...
    def execute_json(self, *params: Union[str, bytes]) -> List:
        ...
        .. warning::
            ...
            This will mimic exiftool's default method of operation "continue on error"
            and "best attempt" to complete commands given.
        ...
        if len(result) == 0:
            ...
            #
            # There's no easy way to check which params are files, or else we have to
            # reproduce the parser exiftool does (so it's hard to detect to raise a
            # FileNotFoundError)

            # Returning [] could be ambiguous if Exiftool changes the returned JSON
            # structure in the future
            # Returning None was preferred, because it's the safest as it clearly
            # indicates that nothing came back from execute(), but it means
            # execute_json() doesn't always return JSON
            # Raising an error is the current solution, as that clearly indicates that
            # you used execute_json() expecting output, but got nothing
        ...
        try:
            ...
            # built-in json.JSONDecodeError is a subclass of ValueError
            ...
            # the user is expected to know this ahead of time, and if -w exists in
            # common_args or as a param, this error will be thrown
