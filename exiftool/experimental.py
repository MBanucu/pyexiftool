...
"""
This submodule contains the ``ExifToolAlpha`` class, which extends the
``ExifToolHelper`` class with experimental functionality.

.. note::
    :py:class:`exiftool.helper.ExifToolAlpha` class of this submodule is available
    in the ``exiftool`` namespace as :py:class:`exiftool.ExifToolAlpha`
...
# Note: They are quite fragile, because this just parses the output
# text from exiftool
def check_ok(result):
    ...
    """
    return result is not None and "due to errors" not in result
...
class ExifToolAlpha(ExifToolHelper):
    """
    This class is for the "experimental" functionality.  In the grand scheme
    of things, this class contains "not well tested" functionality, methods
    that are less used, or methods with niche use cases.  In some methods,
    edge cases on some of these methods may produce unexpected or ambiguous
    results.  However, if there is increased demand, or robustness improves,
    functionality may merge into
    :py:class:`exiftool.ExifToolHelper` class.

    The starting point of this class was to remove all the "less used"
    functionality that was merged in on some arbitrary pull requests to the
    original v0.2 PyExifTool repository.  This alpha-quality code is brittle
    and contains a lot of "hacks" for a niche set of use cases.  As such, it
    may be buggy and it shouldn't crowd the core functionality of the
    :py:class:`exiftool.ExifTool` class or the stable extended functionality
    of the :py:class:`exiftool.ExifToolHelper` class with unneeded bloat.

    The class heirarchy:  ExifTool -> ExifToolHelper -> ExifToolAlpha

    * ExifTool - stable base class with CORE functionality
    * ExifToolHelper - user friendly class that extends the base class with
      general functionality not found in the core
    * ExifToolAlpha - alpha-quality code which extends the ExifToolHelper
      to add functionality that is niche, brittle, or not well tested

    Because of this heirarchy, you could always use/extend the
    :py:class:`exiftool.ExifToolAlpha` class to have all functionality,
    or at your discretion, use one of the more stable classes above.

    Please issue PR to this class to add functionality, even if not tested
    well.  This class is for experimental code after all!
    """

    # ------------------------------------------------------------------
    # i'm not sure if the verification works, but related to pull
    # request (#11)

    def execute_json_wrapper(self, filenames, params=None,
                             retry_on_error=True):
        ...
        if result:
            try:
                ...
                # Restart the exiftool child process in these cases since
            ...
            # Reasons for exiftool to provide an empty result, could be
            # e.g. file not found, etc.
            # What should we do in these cases? We don't have any
            # information what went wrong, therefore we just return
            # empty dictionaries.
    ...
    def get_metadata_wrapper(self, filename, params=None):
        return self.execute_json_wrapper(
            filenames=[filename], params=params)[0]
    ...
    def get_tags_wrapper(self, tags, filename, params=None):
        return self.get_tags_batch_wrapper(tags, [filename],
                                           params=params)[0]
    ...
    # ------------------------------------------------------------------
    # this was a method with good intentions by the original author, but
    # returns some inconsistent results in some cases for example, if you
    # passed in a single tag, or a group name, it would return the first
    # tag back instead of the whole group try calling
    # get_tag_batch("*.mp4", "QuickTime") or "QuickTime:all" ...
    # the expected results is a dictionary but a single tag is returned
    ...
    def get_tag(self, filename, tag):
        ...
        if len(d.values()) > 1:
            raise RuntimeError(
                "multiple tag values returned, invalid use case")
    ...
    def set_keywords_batch(self, files, mode, keywords):
        ...
        * KW_REPLACE: Replace (i.e. set) the full keywords tag with
          `keywords`.
        ...
        * KW_REMOVE:  Remove `keywords` from the keywords tag.
        ...
        if isinstance(keywords, basestring):
            raise TypeError(
                "The argument 'keywords' must be an iterable of strings")

        # allow the files argument to be a str, and process it into a list
        # of str
    ...
    def _check_result_filelist(file_paths, result):
        """
        Checks if the given file paths matches the 'SourceFile' entries in
        the result returned by exiftool. This is done to find possible mix
        ups in the streamed responses.
        """
        # do some sanity checks on the results to make sure nothing was
        # mixed up during reading from stdout
        if len(result) != len(file_paths):
            raise IOError(
                "exiftool returned {} results, but expected was {}".format(
        ...
        for i in range(len(file_paths)):
            ...
            if returned_source_file != requested_file:
                raise IOError(
                    "exiftool returned data for file {}, but expected was "
                    "{}".format(returned_source_file, requested_file))
