...
.. note::
    :py:class:`exiftool.helper.ExifToolHelper` class of this submodule is available in the ``exiftool``
    namespace as :py:class:`exiftool.ExifToolHelper`
...
def _is_iterable(in_param: Any, ignore_str_bytes: bool = False) -> bool:
    ...
    .. note::
        ...
        if you need to consider a code path for strings first, check that before checking if a parameter
        is iterable via this function

        or specify ``ignore_str_bytes=True``

    :param in_param: Something to check if iterable or not
    :param ignore_str_bytes: str/bytes are iterable.  But usually we don't want to check that.  set ``ignore_str_bytes``
        to ``True`` to ignore strings on check
    """
...
class ExifToolHelper(ExifTool):
    ...
    def __init__(self, auto_start: bool = True, check_execute: bool = True, check_tag_names: bool = True, **kwargs) -> None:
        ...
        :param bool check_execute: Will check the exit status (return code) of all commands.  This catches some
            invalid commands passed to exiftool subprocess, defaults to True.  See :py:attr:`check_execute` for more
            info.
        :param bool check_tag_names: Will check the tag names provided to methods which work directly with tag names.
            This catches unintended uses and bugs, default to True.  See :py:attr:`check_tag_names` for more info.

        :param kwargs: All other parameters are passed directly to the super-class constructor:
            :py:meth:`exiftool.ExifTool.__init__()`
        """
    ...
    def execute(self, *params: Any, **kwargs) -> Union[str, bytes]:
        ...
        Adds logic to str() any parameter which is not a str or bytes.  (This allows passing in objects like
        Path _without_ casting before passing it in.)
        ...
        # this was originally to support Path() but it's now generic enough to support any object that str()
        # to something useful
        #
        ...
        # TODO: this list copy could be expensive if the input is a very huge list.
        # Perhaps in the future have a flag that takes the lists in verbatim without any processing?
    ...
    def auto_start(self) -> bool:
        ...
        (There's really no point to having this a read-write property, but allowing a read can be helpful
        at runtime to detect expected behavior.)
        """
    ...
    def check_execute(self) -> bool:
        ...
        If enabled, will raise :py:exc:`exiftool.exceptions.ExifToolExecuteError` if a non-zero exit status is
        returned during :py:meth:`execute()`

        .. warning::
            While this property is provided to give callers an option to enable/disable error checking,
            it is generally **NOT** recommended to disable ``check_execute``.

            **If disabled, exiftool will fail silently, and hard-to-catch bugs may arise.**

            That said, there may be some use cases where continue-on-error behavior is desired.  (Example: dump
            all exif in a directory with files which don't all have the same tags, exiftool returns exit code 1
            for unknown files, but results are valid for other files with those tags)
    ...
    def check_tag_names(self) -> bool:
        ...
        .. warning::
            While this property is provided to give callers an option to enable/disable tag names checking,
            it is generally **NOT** recommended to disable ``check_tag_names``.

            **If disabled, you could accidentally edit a file when you meant to read it.**

            Example: ``get_tags("a.jpg", "tag=value")`` will call ``execute_json("-tag=value", "a.jpg")``
            which will inadvertently write to a.jpg instead of reading it!

            That said, if PH's exiftool changes its tag name regex and tag names are being erroneously
            rejected because of this flag, disabling this could be used as a workaround (more importantly, if this
            is happening, please `file an issue`_!).
    ...
    def get_tags(self, files: Union[Any, List[Any]], tags: Optional[Union[str, List]],  # noqa: C901
    ...
    def set_tags(self, files: Union[Any, List[Any]], tags: Dict, params: Optional[Union[str, List]] = None):  # noqa: C901
        """
        ...
        :param tags: Tag(s) to write.
            ...
            * If a value is a list, will iterate over list and set each individual value to the same tag (
            ...
            .. note::
                Value of the dict can be a list, in which case, the tag will be passed with each item in the list,
                in the order given

                This allows setting things like ``-Keywords=a -Keywords=b -Keywords=c`` by passing in
                ``tags={"Keywords": ['a', 'b', 'c']}``
    ...
    def _check_tag_list(tags: List) -> None:
        ...
        See any notes/warnings in the property :py:attr:`check_tag_names` to get a better understanding
        of what this is for and not for.
        ...
        # However, it appears that "-" may be allowed within a tag name/group
        # (i.e. https://exiftool.org/TagNames/XMP.html Description tags)
        #
        ...
        # * Tag:Tag:tag is not valid, but passes the simple regex (it's ok, this is not supposed to be
        #   a catch-all)... exiftool subprocess accepts it anyways, even if invalid.
