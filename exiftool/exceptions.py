...
class ExifToolProcessStateError(ExifToolException):
    """
    Base class for all errors related to the invalid state of
    `exiftool` subprocess
    """
...
class ExifToolExecuteException(ExifToolException):
    ...
    This exception is never returned directly from any method, but provides
    common interface for subclassed errors.
    ...
    :attribute returncode: Exit Status (Return code) of the ``execute()``
        command which raised the error
    :attribute stdout: STDOUT stream returned by the command which raised
        the error
    :attribute stderr: STDERR stream returned by the command which raised
        the error
    """
...
class ExifToolExecuteError(ExifToolExecuteException):
    ...
    .. note::
        There is a similarly named :py:exc:`ExifToolExecuteException` which
        this Error inherits from.

        That is a base class and never returned directly.  This is what is
        raised.
    """
    ...
    def __init__(self, exit_status, cmd_stdout, cmd_stderr, params):
        super().__init__(
            ...
            exit_status, cmd_stdout, cmd_stderr, params)
...
class ExifToolOutputEmptyError(ExifToolExecuteException):
    """
    ExifTool execute_json() expected output, but execute() did not return
    any output on stdout

    This is an error, because if you expect no output, don't use
    execute_json()
...
class ExifToolJSONInvalidError(ExifToolExecuteException):
    """
    ExifTool execute_json() expected valid JSON to be returned, but got
    invalid JSON.

    This is an error, because if you expect non-JSON output, don't use
    execute_json()
...
class ExifToolVersionError(ExifToolException):
    ...
    PyExifTool is coded to work with a range of exiftool versions.  If the
    advanced params change in functionality and break PyExifTool, this error
    will be thrown
    """
...
class ExifToolTagNameError(ExifToolException):
    ...
    :py:attr:`exiftool.ExifToolHelper.check_tag_names` is enabled, and a bad
    tag is provided to a method
    """
...
class ExifToolServerError(ExifToolException):
    """
    Generic base class for errors related to
    :py:class:`exiftool.server.ExifToolServer`
    """
...
class ExifToolClientError(ExifToolException):
    """
    Generic base class for errors related to
    :py:class:`exiftool.client.ExifToolClient`
    """
