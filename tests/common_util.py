...
def et_get_temp_dir(
    ...
    """
    suffix is optional str to append to the temp dir creation.  This is useful to figure out
    which test created the output folder
    base_path is optional Path to point to the path where the temp dir is to be created.  None=SCRIPT_PATH
    persist is optional bool.  If set to a value, this method will honor that value.
    None = use default up above (useful if you're writing a particular test and you only
    want that one to persist when testing)
