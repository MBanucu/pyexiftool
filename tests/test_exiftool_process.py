...
class TestExifToolProcess(unittest.TestCase):
    ...
    def test_process_died_running_status(self):
        ...
        # There is a very weird bug triggered on WINDOWS only which I've described here:
        # https://exiftool.org/forum/index.php?topic=12472.0
        # it happens specifically when you forcefully kill the process, but at least one
        # command has run since launching, the exiftool wrapper on windows does not terminate
        # the child process
        ...
        # WINDOWS WORKAROUND: take out the method that is called on load (probably not the
        # way to do this well... you can take out this line and watch Python interpreter hang
        # at .kill() below
        if PLATFORM_WINDOWS:
        ...
        self.process.kill()
        # TODO freeze here on windows if there is a zombie process b/c killing immediate
        # exiftool does not kill the spawned subprocess
